...
基礎類 16 課 / 224 題已上線（題數不齊，每課 10~20 題；`COURSE_PLAN.md` §10 第 7 項為準）。**現在要建置進階類 12 課（lesson2-1 ~ lesson2-12），每課 20 題，共 240 題。**

- 題庫檔位置：`docs/data/advanced/lesson2-N.js`（12 課全數完成）
...
| 2-12 | Cargo 生態與實戰慣用法 | D | ✅ 已驗證（含 primer） | 20/20 | ✅ |
...
| Task | 內容 | 狀態 |
|---|---|---|
| T16 | `docs/data/index.js` 12 課 `available` 改 `true` | ✅ 完成 |
| T17 | 全域驗證 + 跨課重複題偵測 + `COURSE_PLAN.md` 狀態更新 | ✅ 完成（留一項待使用者決定，見 §8 最後一則） |
...
### 2026-09-21 — 2-12 上架，進階類 12 課 / 240 題全數完成

**已上架並 push**：2-12（Cargo 生態與實戰慣用法）。**進階類 240/240 完成，T16、T17 一併結案。**

- 987 行、20 題，**沒有任何 `△`**（與骨架預判一致，未用 `desugarChecked`）。反面案例 7/20（第 1、4、6、8、13、15、19 題），符合骨架預期的最低比例。
- 其中 3 題是「概念示範、非 rustc 編譯錯誤」（第 4 workspace 版本統一、13 手改 `Cargo.lock`、15 卡在舊 major），詳解都明講「`cargo build` 會成功、rustc 一句話都不會說，錯的是心智模型」。第 8 題是「能編譯、執行期才 panic」。
- agent 實測範圍很完整：`#[cfg(feature)]` 未啟用的 E0425 gated 訊息、dev/release 的 `debug_assertions` 差異、真的建兩 member workspace 驗 `cargo tree -d`、serde 忘開 `derive` feature 的錯誤原文、clippy/rustfmt 實跑、四種版本範圍的解析結果、四支綜合程式的輸出。
- **依實測修正大綱一處事實錯誤**：骨架第 13 題原寫「lock 與 toml 衝突時 `cargo build` 直接拒絕」——**不成立**。plain `cargo build` 會靜靜重新解析並改回 lock（印 `Updating itoa v0.4.8 -> v1.0.18`），只有 `--locked` 才拒絕。題目與正解已照實測改寫。（`ADVANCED_OUTLINE.md` 第 13 題那一列的敘述未同步修正，那份是施工圖、已完工，不影響線上內容。）

**T17 全域驗證結果**：

- `node scripts/validate-data.js` → 28 課全過（16 基礎 + 12 進階）
- 全站 464 題、id 全域唯一、無重複
- 基礎 224 題 + 進階 240 題 = 464。§1 原本寫「基礎 240 題」是錯的，已依 `COURSE_PLAN.md` §10 第 7 項更正為 224。
- `COURSE_PLAN.md` §10 第 8 項已標 ✅。

**跨課重複題偵測**（雙字元 bigram + 程式碼三字元 shingle 的 Jaccard 相似度，剝除「以下程式碼的結果是」這類樣板句後比對）：

- **2-8、2-12 沒有任何一對被標記**，與既有 26 課都沒撞題。
- 全站標記 8 對可疑，逐對人工看過，**只有 1 對是真重複**：
  - **`1-11-14`（泛型/collect）與 `2-2-09`（迭代器/collect）** —— 程式碼幾乎逐字相同（只差變數名 `nums` / `v`），正解同樣是 `type annotations needed`，考點完全重疊。
  - 其餘 7 對是「同素材、不同考點」或樣板句造成的假陽性，例如 `1-12-19`（為什麼 `Display` 就有 `to_string`）與 `2-9-15`（想自己再實作 `ToString` → conflicting implementations），是合理的遞進；`2-5-20` 與 `2-9-20` 只是總結題句型相同。
- **待使用者決定**：`2-2-09` 要不要改寫成別的考點（例如 `collect` 收成 `HashMap` 或 `String` 的型別標註差異）。兩課都已上線，改動會動到已發布內容，因此沒有自行修改。

**進階類到此完成。** 後續若要繼續，下一個階段是補充教材 lesson2-13 ~ 2-20（`COURSE_PLAN.md` §5，8 課），目前 `index.js` 全部 `available: false`。
