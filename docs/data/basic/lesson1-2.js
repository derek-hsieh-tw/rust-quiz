...
整數同理:i32 + i64 也不行,必須用 as 或 From 顯式轉換,例如 x + (y as f64)。這是 Rust 消滅「隱式轉換造成的精度陷阱」的手段——所有型別轉換都必須看得見。`,
    ...
    {
      id: "1-2-11",
      question: "字面值的兩個小語法。以下程式碼的結果是?",
      questionCode: "fn main() {\n    let population = 1_000_000;\n    let single = (5,);\n    let grouped = (5);\n\n    println!(\"{} {:?} {}\", population, single, grouped);\n}",
      options: [
        { text: "印出 1000000 (5,) 5——底線只是給人看的分隔符,(5,) 是單元素 tuple,(5) 只是加了括號的 5" },
        { text: "編譯錯誤:數字字面值裡不能出現底線" },
        { text: "編譯錯誤:(5,) 結尾多了一個逗號" },
        { text: "印出 1_000_000 (5,) (5)——底線與括號都會原樣保留" },
      ],
      answer: 0,
      explanation: `兩個獨立的小語法,共通點是「純粹為了讓人讀得懂」:
數字底線:編譯器直接忽略它,1_000_000 與 1000000 產生完全相同的位元組。它可以放在任何位置,所以也能用來分組非十進位字面值,例如 0xFF_FF_FF、0b1010_0011。字面值後面還能直接黏型別後綴:1_000u64、3.5f32。
單元素 tuple 的結尾逗號:(5) 在 Rust(跟大多數語言一樣)只是「被括號包住的 5」,型別是 i32;要表達「只有一個元素的 tuple」必須寫成 (5,),型別是 (i32,)。這個逗號不是筆誤,是唯一能區分兩者的標記。日常很少手寫,但看 macro 展開、函式回傳單元素 tuple、或 zip 的結果時會遇到,認得出來才不會以為是打錯字。
      ...
      walkthrough: [
        {
          ...
          lines: [
            ...
            { code: "    let single = (5,);", note: "結尾那個逗號讓它成為「單元素 tuple」,型別是 (i32,),用 single.0 取值。" },
            { code: "    let grouped = (5);", note: "沒有逗號,括號只是分組用,這行等同 let grouped = 5;,型別是 i32。" },
            { code: "", note: "" },
            { code: "    println!(\"{} {:?} {}\", population, single, grouped);", note: "印出 1000000 (5,) 5。注意 single 用的是 {:?}——tuple 沒有 Display 實作,只能用 Debug 印(這個區別見 lesson1-7)。" },
            { code: "}", note: "main 結束。" },
          ],
          outro: "底線也能用在非十進位字面值:0xFF_FF_FF、0b1010_0011、1_000u64 都合法。",
        },
      ],
      csharp: `數字分隔符兩邊一樣:C# 7.0 起也支援 1_000_000,連 0b1010_0011 的寫法都相同。單元素 tuple 則是 C# 沒有的東西——ValueTuple 至少要兩個元素,(5) 在 C# 也只是加括號的 5,而且沒有任何語法能表達「一個元素的 tuple」。Rust 因為 tuple struct 與巨集展開常會產生單元素 tuple,必須有辦法把它跟括號區分開,結尾逗號就是那個辦法。`,
    },
  ],
};
