...
檢查全部發生在編譯期,「執行期檢查比較慢」的說法正好說反。`,
    ...
    {
      id: "1-8-11",
      question: "let-else 敘述。以下程式碼的結果是?",
      questionCode: "fn parse_port(text: &str) -> u16 {\n    let Ok(port) = text.parse::<u16>() else {\n        return 8080;\n    };\n\n    port\n}\n\nfn main() {\n    println!(\"{}\", parse_port(\"3000\"));\n    println!(\"{}\", parse_port(\"abc\"));\n}",
      options: [
        { text: "印出 3000 與 8080:比對成功時 port 被綁定到「外層」作用域繼續使用;失敗時執行 else 區塊,而該區塊必須離開當前流程(return / break / continue / panic!)" },
        { text: "印出 3000 與 0:else 區塊的作用是提供一個預設值給 port" },
        { text: "編譯錯誤:let 後面不能接 else" },
        { text: "兩次都印出 8080:只要寫了 else,比對結果就一律走 else 分支" },
      ],
      answer: 0,
      explanation: `let-else 是 Rust 1.65 起的語法,專門服務 guard clause(防衛式子句)這個模式:「拿不到我要的東西就趕快離開,拿得到就繼續往下走」。
它跟 if let 的差別只有一個,但很關鍵——綁定的位置。if let Ok(port) = ... { ... } 的 port 只活在大括號裡,想在後面繼續用就得把整段邏輯往裡面搬,連續幾個這種判斷就會疊成一座金字塔;let-else 的 port 綁在外層,失敗的那條路被 else 帶走了,主線因此保持在最左邊,一層縮排都不用加。
關鍵限制:else 區塊必須「發散」——return、break、continue、panic!、process::exit 之類,總之不能正常結束。理由很直接:如果 else 能正常走完,執行流會抵達下一行,但那時 port 從來沒有被綁定過。所以 else 區塊不是用來「提供預設值」的(那是 unwrap_or 的工作),它是用來離開的。
      ...
      walkthrough: [
        {
          ...
          lines: [
            ...
            { code: "        return 8080;", note: "比對失敗才走這裡。這一行必須離開函式——else 區塊不能正常結束,否則下一行會用到從沒綁定過的 port。" },
            { code: "    };", note: "注意這裡有分號:let-else 整體是一個 let 敘述,不是 if 運算式。" },
            { code: "", note: "" },
            { code: "    port", note: "從這行起 port 是一個普通的 u16,沒有任何縮排,也不需要再解包一次。" },
            { code: "}", note: "函式結束。" },
            { code: "", note: "" },
            { code: "fn main() {", note: "程式進入點。" },
            { code: "    println!(\"{}\", parse_port(\"3000\"));", note: "\"3000\" 解析成功,走主線,印出 3000。" },
            { code: "    println!(\"{}\", parse_port(\"abc\"));", note: "\"abc\" 解析失敗,走 else 提早回傳,印出 8080。" },
            { code: "}", note: "main 結束。" },
          ],
        },
        {
          ...
          lines: [
            ...
            { code: "        8080", note: "⛔ 編譯失敗:`else` clause of `let...else` does not diverge,後面還跟著一句 expected `!`, found integer——那個 ! 就是下面去糖區塊講的 never type。這裡想「回傳預設值給 port」,但 let-else 沒有這種用法——比對失敗代表 port 拿不到值,唯一的出路是離開。" },
            { code: "    };", note: "let-else 結束。" },
            { code: "", note: "" },
            { code: "    port", note: "如果上面那個 else 被允許正常結束,執行流會走到這裡,而 port 從來沒有被綁定過——編譯器不可能放行。" },
            { code: "}", note: "函式結束。" },
          ],
        },
        {
          ...
          lines: [
            ...
            { code: "}", note: "函式結束。" },
            { code: "", note: "" },
            { code: "fn load(text: &str) -> u16 {", note: "那 let-else 什麼時候才划算?當失敗要做的事「不只是換個值」的時候。" },
            { code: "    let Ok(port) = text.parse::<u16>() else {", note: "例如:失敗要記錄、要回傳錯誤、要提早結束。" },
            { code: "        eprintln!(\"無法解析埠號:{text},改用預設值\");", note: "先做點別的事——這是 unwrap_or 做不到的。" },
            { code: "        return 8080;", note: "然後離開。else 區塊只要最後有離開就合法,中間想做幾件事都行。" },
            { code: "    };", note: "let-else 結束。" },
            { code: "", note: "" },
            { code: "    port", note: "主線維持零縮排。" },
            { code: "}", note: "函式結束。" },
            { code: "", note: "" },
            { code: "fn main() {", note: "程式進入點。" },
            { code: "    println!(\"{} {}\", parse_port(\"abc\"), load(\"abc\"));", note: "兩種寫法都印出 8080,但後者順便留下了一行診斷訊息。" },
            { code: "}", note: "main 結束。" },
          ],
        },
        {
          ...
          lines: [
            ...
            { code: "let port = match text.parse::<u16>() {", note: "本體是一個 match,而且它的值要賦給 port。" },
            { code: "    Ok(v) => v,", note: "成功分支:值的型別是 u16。" },
            { code: "    Err(_) => return 8080,", note: "失敗分支:return 的型別是 never type(寫作 !),代表「這裡不會產生值,執行流就此離開」。never 型別可以塞進任何位置,所以兩條分支的型別對得上。" },
            { code: "};", note: "展開結束,port 的型別是 u16。" },
          ],
          outro: "關鍵就在那個 never type:match 的每條分支型別必須一致,失敗分支若不是 return 這類「不回傳」的運算式,就得產出一個 u16——而它拿不出來。這就是 does not diverge 這句錯誤訊息的來源。",
        },
      ],
      csharp: `這正是 C# 的防衛式子句 if (!int.TryParse(text, out var port)) return 8080;——連「解構出來的變數在後面繼續可用」這點都一樣(out var 的作用域也會延伸到外層)。差別在強制力:C# 的 if 區塊裡想寫什麼都行,忘了 return 就會掉到下一行拿著未定值繼續跑;Rust 的 else 區塊若沒有離開就編譯失敗,防衛式子句這個慣例在這裡變成了語言規則。`,
    },
  ],
};
